...
> [!WARNING]  
> This project is currently configured for development use only.
...
> * Production configuration (e.g., Apache, hardened Django settings) is not included
> 
> A production deployment would typically require additional setup such as:
> * A production-ready Apache configuration (e.g., `mod_wsgi`)
> * Secure handling of environment secrets and configuration
> * Proper Django production settings (`DEBUG=False`, `ALLOWED_HOSTS`, etc.)
...
## Common Problems
* Running `docker-compose` in Linux machines have to be used with `sudo`. Without it you'll have permissions issues
   with docker
